...
class Gui:
    ...
    def predict_from_frame(self, image):
        ...
        if self.last_prediction != self.prediction:
            self.last_prediction = self.prediction
            self.launch_program(self.prediction)

    def launch_program(self, prediction):
        """
        Launch a given program based on a prediction.
        :param prediction: integer specifing the number of fingers shown
        :return: None
        """
        if prediction == "3":
            ...
            subprocess.Popen(["notepad", "testing.txt"])
            self.is_program_launched = True
